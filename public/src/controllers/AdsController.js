...
module.exports = {
  ...
  addAction: async (req, res) => {
    ...
    if (!title || !cat) {
      res.json({ error: 'Titulo e/ou categoria nao foram preenchidos!' });
      return;
    }
    ...
    if (req.files && req.files.img) {
      if (req.files.imag.length == undefined) {
  ...
  editAction: async (req, res) => {
    let { id } = req.params;
    let { title, price, priceneg, desc, cat, images, token } = req.body;
    ...
    if (user._id.toString() !== ad.idUser) {
      res.json({ error: 'esse anuncio nao é seu' });
      return;
